...
class AttentionLoss(loss._Loss):
    """The KL-Divergence loss for the binary student model and real teacher output.
    ...
    def forward(self, model_output, real_output):
        # model_output: student, real_output: teacher
        ...
        qs = self.normalized_attention_vector(model_output)
        qt = self.normalized_attention_vector(real_output)

        att_loss = torch.mean((qs-qt).pow(2), -1)
    ...
    def normalized_attention_vector(self, act_vol):
        # (N, C, H, W) -> (N, H, W, C)
        act_vol = act_vol.permute(0,2,3,1)
        att_area = torch.mean(act_vol.pow(2), -1)
        batch_size = att_area.size(0)
        att_vec = att_area.view(batch_size,-1)
        return att_vec / torch.mean(att_vec, -1, keepdim=True)
